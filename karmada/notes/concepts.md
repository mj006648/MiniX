...
> 목적: MiniX에서 먼저 Karmada를 실험하고, 이후 TWINX / EDGEX / DATAX / TOWER 구조로 확장하기 위한 개념 정리.
...
GitHub MiniX repo
  -> ArgoCD
    -> Karmada API Server
      -> TWINX / EDGEX / DATAX / 기타 member cluster
```
...
→ TWINX, EDGEX, DATAX처럼 목적이 다른 자체 클러스터를 묶는 데도 적합하다.
...
우리 계획에서는 `TOWER CLUSTER`가 여기에 해당한다.
...
→ `minix.io/pool=gpu` 라벨이 붙은 클러스터로 배포한다.

MiniX 장기 라벨 예시:

```text
minix.io/role=tower
minix.io/role=gpu-render
minix.io/role=edge-gpu
minix.io/role=data
minix.io/pool=gpu
minix.io/pool=cpu
minix.io/storage=ssd
minix.io/storage=hdd
minix.io/location=homelab
```
...
```text
TWINX cluster taint: maintenance=true:NoSchedule
Policy가 tolerate 안 함
-> 새 workload는 TWINX로 안 감
-> 필요 시 다른 클러스터로 failover
```
...
```text
GPU rendering app
  -> minix.io/role=gpu-render 클러스터 우선
  -> 부족하면 minix.io/pool=gpu 클러스터로 overflow
  -> EDGEX는 latency-sensitive 작업만 허용
```

### 13.3 Cross-cluster failover

클러스터 장애 또는 cluster taint에 따라 replica를 다른 클러스터로 옮길 수 있다.
...
```text
               ...
               |                  |                  |
               v                  v                  v
        TWINX Cluster        EDGEX Cluster       DATAX Cluster
      GPU Rendering 전용    Edge GPU Compute     PB급 SSD/HDD Data
               |
               v
        RESOURCE POOL
       GPU Pool / CPU Pool
...
```text
minix.io/role=edge-gpu
minix.io/location=edge
minix.io/latency=low
```
